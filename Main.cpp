...
struct My_Patches {
    ...
    MemoryPatch Tickets, Coins, Gems;
    // etc...
} hexPatches;
...
Java_uk_lgl_MainActivity_Toast(JNIEnv *env, jclass obj, jobject context) {
    MakeToast(env, context, OBFUSCATE("Modded by JTH MODz"), Toast::LENGTH_LONG);
}
...
Java_uk_lgl_modmenu_FloatingModMenuService_getFeatureList(JNIEnv *env, jobject activityObject) {
    ...
    const char *features[] = {
            OBFUSCATE("Category_JTH MODz"), //Not counted
            OBFUSCATE("Toggle_Tickets"), //case 0
            OBFUSCATE("Toggle_Coins"), //case 1
            OBFUSCATE("Toggle_Gems"), //case 2
    };
...
Java_uk_lgl_modmenu_Preferences_Changes(JNIEnv *env, jclass clazz, jobject obj,
    ...
    switch (featNum) {
        case 0:
            feature2 = boolean;
            if (feature2) {
                hexPatches.Tickets.Modify();
            } else {
                hexPatches.Tickets.Restore();
            }
            break;
        ...
        case 3:
            feature2 = boolean;
            if (feature2) {
                hexPatches.Gems.Modify();
            } else {
                hexPatches.Gems.Restore();
            }
            break;
    }
}
...
void *hack_thread(void *) {
    ...
    hexPatches.Tickets= MemoryPatch::createWithHex(targetLibName,
                                                    string2Offset(OBFUSCATE_KEY("0x1777468", '3')),
                                                    OBFUSCATE("02 01 E0 E3 1E FF 2F E1"));
    //You can also specify target lib like this
    hexPatches.Coins = MemoryPatch::createWithHex("libtargetLibHere.so",
                                                     string2Offset(OBFUSCATE_KEY("0x17895AC", 'g')),
                                                     OBFUSCATE("02 01 E0 E3 1E FF 2F E1"));
    //You can also specify target lib like this
    hexPatches.Gems = MemoryPatch::createWithHex("libtargetLibhere.so",
                                                  string2offset(OBFUSCATE_KEY("0x177AAF4", 'l')),
                                                  OBFUSCATE("02 01 E0 E3 1E FF 2F E1"));
    // Offset Hook example
    ...
    // New way to patch hex via KittyMemory without need to specify len. Spaces or without spaces are fine
    hexPatches.Tickets = MemoryPatch::createWithHex(targetLibName,
                                                    string2Offset(OBFUSCATE_KEY("0x17895AC", '-')),
                                                    OBFUSCATE("00 00 A0 E3 1E FF 2F E1"));
    //You can also specify target lib like this
    hexPatches.Tickets = MemoryPatch::createWithHex("libtargetLibHere.so",
